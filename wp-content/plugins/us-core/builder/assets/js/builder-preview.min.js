...
return}
$usb.builder.removeMovedElement(baseCurrentId);self._maybeDrop(self._extractEventData(e));$usb.builder.setTransitPosition(transit.pageX,transit.pageY)}},_maybeDrop:function(data){const self=this;if(!data||!data.target||!$usb.builder.isMode('drag:add','drag:move')||$usb.builder.hasDragScrolling()){return}
...
var currentCursorDirectionInTarget=self._getCurrentCursorDirectionInTarget(self.getElmNode(children[currentIndex]),data.clientX,data.clientY,_2D_AXES_[only_x_axis?'x':'y']);if([_DIRECTION_.BOTTOM,_DIRECTION_.RIGHT].includes(currentCursorDirectionInTarget)){currentIndex+=1}}
...
const insert=$usb.builder.getInsertPosition(targetContainerId,currentIndex);if($ush.comparePlainObject(insert,dragData.lastInsert)){return}
...
if(dragData.isParentTab&&aliasId){if($usb.builder.isValidId(insert.parent)){insert.parent=['prepend','append'].includes(insert.position)?self.getElmNode(currentId).parentNode||targetContainer:$usb.builder.addAliasToElmId(aliasId,insert.parent)}}
...
const dragData=$usbcore.cache('iframeDrag').data();if($usb.builder.isAliasElmId(dragData.currentId)){dragData.currentId=$usb.builder.removeAliasFromId(dragData.currentId)}
...
return}
if(!!dragData.parentId&&!!dragData.currentId){$usb.builder.moveElm(dragData.currentId,dragData.parentId,dragData.currentIndex||0);if($usb.builder.isElmSection(dragData.parentId)){self.openSectionById(dragData.parentId)}
...
if(self.selectedElmId){self.hideEditableHighlight()}},_elmAnimationEnd:function(e){const self=this;if(!$usbcore.$attr(e.target,'data-usbid')){return}
if($usb.builder.isValidId(self.selectedElmId)){self.showEditableHighlight(self.selectedElmId);self.setHighlightsPosition()}},_mouseLeavesIframe:function(e){if($usb.panel.isShow()){this.hideHighlight()}},_elmSelected:function(e){const self=this;if(!$usb.builder.isMode('editor')||!$usb.panel.isShow()||$usbcore.$hasClass(e.target,'usb_skip_elmSelected')){return}
...
if(!$style){var
customPrefix=$usb.config('designOptions.customPrefix',defaultCustomPrefix),className=$ush.uniqid(customPrefix);if(!$ush.isNode(node)){return}
