...
history.pushState($.extend(state,self.getChangedParams()),'',lastUrl=self.toString());$window.trigger('pushstate');return self}})}}(jQuery);window.$ush=window.$ush||{};window.$usof=window.$usof||{};jQuery.fn.usMod=function(mod,value){if(this.length==0){return this}
if(value===!1||value===null){return this.each(function(){this.className=this.className.replace(new RegExp('(^| )'+mod+'\_[\dA-z\_\-]+( |$)'),'$2')})}
var pcre=new RegExp('^.*?'+mod+'\_([\dA-z\_\-]+).*?$'),arr;if($ush.isUndefined(value)){return(arr=pcre.exec(this.get(0).className))?arr[1]:!1}else{var regexp=new RegExp('(^| )'+mod+'\_[\dA-z\_\-]+( |$)');return this.each(function(){if(this.className.match(regexp)){this.className=this.className.replace(regexp,'$1'+mod+'_'+value+'$2')}else{this.className+=' '+mod+'_'+value}}).trigger('usof.'+mod,value)}};!function($,_undefined){"use strict";$usof.ajaxUrl=$('.usof-container').data('ajaxurl')||window.ajaxurl;if($ush.isUndefined($usof.mixins)){$usof.mixins={}}
$usof.mixins.Events={on:function(eventType,handler){const self=this;if($ush.isUndefined(self.$$events)){self.$$events={}}(eventType+'').split(/\p{Zs}/u).map((_eventType)=>{if($ush.isUndefined(self.$$events[_eventType])){self.$$events[_eventType]=[]}
if(typeof handler==='function'){self.$$events[_eventType].push(handler)}else{console.error('Invalid handler:',[_eventType,handler])}});return self},off:function(eventType,handler){const self=this;if($ush.isUndefined(self.$$events)||$ush.isUndefined(self.$$events[eventType])){return self}
...
return self},has:function(eventType){return!$ush.isUndefined(this.$$events[eventType])&&this.$$events[eventType].length>0},trigger:function(eventType,extraParameters){const self=this;if($ush.isUndefined(self.$$events)||$ush.isUndefined(self.$$events[eventType])||self.$$events[eventType].length==0){return self}
const args=arguments;const params=(args.length>2||!Array.isArray(extraParameters))?Array.prototype.slice.call(args,1):extraParameters;params.unshift(self);for(var i=0;i<self.$$events[eventType].length;i ++){self.$$events[eventType][i].apply(self.$$events[eventType][i],params)}
return self}};$usof.mixins.Fieldset=$.extend({},$ush.mixinEvents,{initFields:function($container){const self=this;['$fields','fields','groups','showIf','showIfDeps'].map((prop)=>{if(!$.isPlainObject(self[prop])){self[prop]={}}});$('.usof-form-row, .usof-form-wrapper, .usof-form-group',$container).each((_,node)=>{var $field=$(node),name=$field.data('name'),isRow=$field.hasClass('usof-form-row'),isGroup=$field.hasClass('usof-form-group'),isInGroup=$field.parents('.usof-form-group').length,$showIf=$field.find((isRow||isGroup)?'> .usof-form-row-showif':'> .usof-form-wrapper-content > .usof-form-wrapper-showif');if($ush.isUndefined(name)){return}
if($field.closest('.usof-form-group-prototype').length){return}
...
self.$fields[name]=$field;if($showIf.length>0){self.showIf[name]=$showIf[0].onclick()||[];const showIfVars=self._getShowIfVariables(self.showIf[name]);for(var i=0;i<showIfVars.length;i ++){if($ush.isUndefined(self.showIfDeps[showIfVars[i]])){self.showIfDeps[showIfVars[i]]=[]}
...
if($ush.isUndefined(self._defaultValues)){self._defaultValues=self.getValues()}},updateVisibility:function(fieldName,isAnimated,isCurrentShown){const self=this;if(!fieldName||!self.showIfDeps[fieldName]){return}
if($ush.isUndefined(isAnimated)){isAnimated=!0}
if($ush.isUndefined(isCurrentShown)){isCurrentShown=!0}
const isPrevShown=self.$fields[fieldName].data('isShown');self.showIfDeps[fieldName].map((depFieldName)=>{var field=self.fields[depFieldName]||self.groups[depFieldName],$field=self.$fields[depFieldName],isShown=self.getCurrentShown(depFieldName),condition=self.showIf[depFieldName],shouldBeShown=self.executeShowIf(condition,self.getValue.bind(self));if((!shouldBeShown&&isShown)||!isCurrentShown){isShown=!1}else if(shouldBeShown&&!isShown){isShown=!0}
if(!$ush.isUndefined(isPrevShown)){isShown=isPrevShown&&isShown}
if(condition.length===3&&condition[1]==='or'){isShown=isShown||shouldBeShown}
...
if(self.showIfDeps[depFieldName]){self.updateVisibility(depFieldName,isAnimated,isShown)}})},getCurrentShown:function(fieldName){const self=this;if(!fieldName||!self.$fields[fieldName]){return!0}
var $field=self.$fields[fieldName],isShown=$field.data('isShow');if($ush.isUndefined(isShown)){isShown=$field.css('display')!=='none'}
return isShown===!0},_getShowIfVariables:function(condition){const self=this;if(!Array.isArray(condition)||condition.length<3){return[]}else if(['and','or'].includes(condition[1].toLowerCase())){var vars=self._getShowIfVariables(condition[0]),index=2;while(!$ush.isUndefined(condition[index])){vars=vars.concat(self._getShowIfVariables(condition[index]));index=index+2}
return vars}else{return[condition[0]]}},executeShowIf:function(condition,getValue){const self=this;var result=!0;if(!Array.isArray(condition)||condition.length<3){return result}else if(['and','or'].includes(condition[1].toLowerCase())){result=self.executeShowIf(condition[0],getValue);var index=2;while(!$ush.isUndefined(condition[index])){condition[index-1]=condition[index-1].toLowerCase();if(condition[index-1]=='and'){result=(result&&self.executeShowIf(condition[index],getValue))}else if(condition[index-1]=='or'){result=(result||self.executeShowIf(condition[index],getValue))}
index=index+2}}else{const value=getValue(condition[0]);if($ush.isUndefined(value)){return!0}
if(condition[1]=='='){if(Array.isArray(condition[2])){result=condition[2].includes(value)}else{result=(value==condition[2])}}else if(condition[1]=='!='){if(Array.isArray(condition[2])){result=!condition[2].includes(value)}else{result=(value!=condition[2])}}else if(condition[1]=='<='){result=(value<=condition[2])}else if(condition[1]=='<'){result=(value<condition[2])}else if(condition[1]=='>'){result=(value>condition[2])}else if(condition[1]=='>='){result=(value>=condition[2])}else if(condition[1]=='str_contains'){result=(''+value).indexOf(''+condition[2])>-1}else{result=!0}}
return result},fireFieldEvent:function($container,trigger){if(!$container.hasClass('usof-form-row')){$('.usof-form-row',$container).each((_,row)=>{var $row=$(row),isShown=$row.data('isShown');if($ush.isUndefined(isShown)){isShown=$row.css('display')!='none'}
if(!isShown&&['beforeShow','afterShow','beforeHide','afterHide'].includes(trigger)){return}
if($ush.isUndefined($row.data('usofField'))){return}
$row.data('usofField').trigger(trigger)})}else if($container.data('usofField')instanceof $usof.field){$container.data('usofField').trigger(trigger)}},getValue:function(id){const self=this;if($ush.isUndefined(self.fields[id])){return _undefined}
return self.fields[id].getValue()},setValue:function(id,value,quiet){const self=this;if($ush.isUndefined(self.fields[id])){return}
const shouldFireShow=!self.fields[id].inited;if(shouldFireShow){self.fields[id].trigger('beforeShow');self.fields[id].trigger('afterShow')}
self.fields[id].setValue(value,quiet);if(shouldFireShow){self.fields[id].trigger('beforeHide');self.fields[id].trigger('afterHide')}},getValues:function(){const self=this;const values={};for(const fieldId in self.fields){if(!self.fields.hasOwnProperty(fieldId)){continue}
values[fieldId]=self.getValue(fieldId)}
for(const groupId in self.groups){values[groupId]=self.groups[groupId].getValue()}
return values},setValues:function(values,quiet){const self=this;for(const fieldId in self.fields){if(values.hasOwnProperty(fieldId)){const currentValue=values[fieldId];self.setValue(fieldId,currentValue,quiet);if(!quiet){self.fields[fieldId].trigger('change',[currentValue])}}else if(self._defaultValues.hasOwnProperty(fieldId)){self.setValue(fieldId,self._defaultValues[fieldId],quiet)}}
for(const groupId in self.groups)if(!$ush.isUndefined(values[groupId])){self.groups[groupId].setValue(values[groupId])}
if(quiet){for(const fieldName in self.showIfDeps){if(!self.showIfDeps.hasOwnProperty(fieldName)||$ush.isUndefined(self.fields[fieldName])){continue}
self.updateVisibility(fieldName,!1);}}},getCurrentValues:function(){const self=this;const result={};for(const name in self.fields){result[name]=self.fields[name].getCurrentValue()}
for(const name in self.groups){result[name]=self.groups[name].getCurrentValue()}
...
return iconTag},initDataIndicator:function(){const self=this;const tmpDataIndicator={params:{},tabIndexes:{},lastChecksum:0,}
const changeField=(theField)=>{if(!(theField instanceof $usof.field)){return}
tmpDataIndicator.params[theField.name]=theField.hasValue();var checksum=$ush.checksum(tmpDataIndicator.params);if(tmpDataIndicator.lastChecksum===checksum){return}
tmpDataIndicator.lastChecksum=checksum;var hasValues={};for(const name in tmpDataIndicator.tabIndexes){var tabIndex=tmpDataIndicator.tabIndexes[name];hasValues[tabIndex]=hasValues[tabIndex]||tmpDataIndicator.params[name]}
self.trigger('dataIndicatorChanged',hasValues);};for(const name in self.fields){const theField=self.fields[name];if(theField.checkParamForDataIndicator){tmpDataIndicator.params[name]=theField.hasValue();tmpDataIndicator.tabIndexes[name]=theField.$row.closest('.usof-tabs-section').index();theField.on('change',changeField)}}
for(const name in self.groups){const theGroup=self.groups[name];if(theGroup.checkParamForDataIndicator){tmpDataIndicator.params[name]=theGroup.hasValue();tmpDataIndicator.tabIndexes[name]=theGroup.$container.closest('.usof-tabs-section').index();theGroup.on('change',changeField)}}},});if($ush.isUndefined($usof._$$data)){$usof._$$data={}}
$usof.getData=function(key){const self=this;if(typeof key!=='string'){return{}}
if(!$.isPlainObject(self._$$data[key])){try{self._$$data[key]=JSON.parse(self._$$data[key]||'{}')}catch(e){self._$$data[key]={}}}
return $ush.clone(self._$$data[key]||{})}}(jQuery);!function($,_undefined){"use strict";$usof.field=function(row,options){const self=this;self.$document=$(document);self.$row=$(row);self.$responsive=$('> .usof-form-row-responsive',self.$row);const data=self.$row.data()||{};self.type=self.$row.usMod('type');self.id=data.id;self.uniqid=$ush.uniqid();self.name=data.name;self.inited=!!data.inited;self.relatedOn=data.relatedOn;self.checkParamForDataIndicator=self.$row.hasClass('check_param_for_data_indicator');self.$input=$(`[name="${data.name}"]:not(.js_hidden)`,self.$row);if(self.inited){return}
self.$$events={beforeShow:[],afterShow:[],change:[],beforeHide:[],afterHide:[]};if(!$ush.isUndefined($usof.field[self.type])){for(const fn in $usof.field[self.type]){if(!$usof.field[self.type].hasOwnProperty(fn)||fn.substr(0,2)==='_$'){continue}
if(!$ush.isUndefined(self[fn])){const parentFn='parent'+fn.charAt(0).toUpperCase()+fn.slice(1);self[parentFn]=self[fn]}
self[fn]=$usof.field[self.type][fn]}}
self.$document.on('usb.syncResponsiveState',self._usbSyncResponsiveState.bind(self));self.$row.data('usofField',self);function initEvent(){self.init(options);self.inited=!0;self.$row.data('inited',self.inited);self.off('beforeShow',initEvent);self._std=data.hasOwnProperty('std')?data.std:self.getCurrentValue();self.initResponsive()};self.on('beforeShow',initEvent)};$.extend($usof.field.prototype,$usof.mixins.Events,{init:function(){const self=this;if($ush.isUndefined(self._events)){self._events={}}
self._events.change=()=>{self.trigger('change',[self.getValue()])};self.$input.on('change',self._events.change);return self},isLiveBuilder:function(){return this.$row.closest('.usb-panel-fieldset, .usb-panel-body').length>0},initResponsive:function(){const self=this;if(!self.hasResponsive()){return}
...
self.$switchResponsive.on('click',self._$switchResponsive.bind(self));self.$responsive.on('click','[data-responsive-state]',self._$selectResponsiveState.bind(self))},hasResponsive:function(){return this.$responsive.length>0},isResponsive:function(){return this.hasResponsive()&&this.$row.hasClass('responsive')},isResponsiveValue:function(value){const self=this;if(value){if(self.isObjectValue(value)){value=$ush.toPlainObject(value)}
if($.isPlainObject(value)){for(const i in self._states){if(value.hasOwnProperty(self._states[i])){return!0}}}}
return!1},isObjectValue:function(value){return value&&$ush.toString(value).indexOf($ush.rawurlencode('{'))===0},isValidState:function(state){return state&&(this._states||[]).includes($ush.toString(state))},isWPBakeryParamValue:function(){return this.$input.hasClass('wpb_vc_param_value')},isDynamicVariable:function(value){return value&&/^{{([\dA-z\/\|\-_]+)}}$/.test($ush.toString(value).trim())},hasValue:function(){return this.getValue()!==this.getDefaultValue()},getParent:$.noop,getRelatedField:function(){const self=this;const parent=self.getParent();if(!$ush.isUndefined(self.relatedOn)&&parent instanceof $usof.GroupParams){return(parent.fields||{})[$ush.toString(self.relatedOn)]}
return _undefined},getFieldByName:function(name){if(name){return((this.getParent()||{})['fields']||{})[name]}
return},getCurrentState:function(){const self=this;if(!self.isValidState(self._currentState)){self._currentState='default'}
return self._currentState},getDefaultValue:function(){return $ush.isUndefined(this._std)?'':this._std},getValueByState:function(state,value){const self=this;if(self.isResponsiveValue(value)){if(!self.isValidState(state)){state='default'}
if(!$.isPlainObject(value)){value=$ush.toPlainObject(value)}
if(value.hasOwnProperty(state)){return value[state]}}
return self.getDefaultValue()},setValueByState:function(state,input,value){const self=this;if(!self.isValidState(state)){return ''}
...
if(self.isWPBakeryParamValue()&&self.$input.is(':hidden')){self.$input.trigger('change')}},getValue:function(){return this.$input.val()},setValue:function(value,quiet){const self=this;if(!self.isResponsive()&&self.isResponsiveValue(value)){self.$row.addClass('responsive')}
self.$input.val(value);if(!quiet){self.trigger('change',[value])}
if(self.isWPBakeryParamValue()){self.$input.trigger('change')}},_usbSyncResponsiveState:function(_,state){const self=this;state=state||'default';if(!self.isResponsive()||!self.isValidState(state)){return}
self._$setResponsiveState(state)},_$setResponsiveState:function(state){const self=this;if(!self.hasResponsive()){return}
if(!self.isValidState(state)){state='default'}
self.$responsiveButtons.removeClass('active').filter(`[data-responsive-state="${state}"]`).addClass('active');self._currentState=state;self.trigger('setResponsiveState',state)},_$switchResponsive:function(){const self=this;if(!self.hasResponsive()){return}
const nextMode=!self.isResponsive();self.$row.toggleClass('responsive',nextMode);var value=self.getCurrentValue();if(nextMode){var responsiveValue={};self._states.map((state)=>{responsiveValue[state]=value});value=$ush.toString(responsiveValue)}else{self._$setResponsiveState('default');if($.isPlainObject(value)){value=$ush.toString(value)}}
self.setValue(value)},_$selectResponsiveState:function(e){const self=this;if(!self.isResponsive()){return}
const state=$(e.target).data('responsive-state')||self._currentState;self._$setResponsiveState(state);self.trigger('syncResponsiveState',state);self.$document.trigger('field.syncResponsiveState',state)}});$.fn.usofField=function(options){return new $usof.field(this,options)}}(jQuery);!function($,_undefined){"use strict";$usof.Group=function(row,options){this.init(row,options)};const _filtersHandler={sanitize_color_slug:function(value){if(value.charAt(0)!=='_'){value='_'+value}
return $ush.toLowerCase(value).replace(/[\p{Zs}|-]+/gu,'_').replace(/[^a-z\d\_]+/g,'').replace(/[\_]+/g,'_')},unique_value:function(value,reserved_values,usofField){const self=this;const name=usofField.name;const values=$ush.toArray(reserved_values);self.groupParams.map((groupParams)=>{if(groupParams.fields[name]===usofField){return}
const value=groupParams.fields[name].getCurrentValue();if(value){values.push(value)}});if(values.indexOf(value)>-1){value=(value.match(/(.*)([-_\p{Zs}]\d+)$/u)||[])[1]||value;const separator=($ush.toPlainObject(self._filters[name]).sanitize_color_slug?'_':' ');var i=1;while(i++<=1000){const newValue=value+separator+i;if(values.indexOf(newValue)<0){value=newValue;break}}}
return value},};$.extend($usof.Group.prototype,$usof.mixins.Events,{init:function(container,options){const self=this;self.$container=$(container);self.$btnAddGroup=$('.usof-form-group-add',container);self.$prototype=$('.usof-form-group-prototype',container);self.groupName=self.$container.data('name');self.groupParams=[];self._filters={};self.isBuilder=self.$container.parents('.us-bld-window').length>0;self.isLiveBuilder=self.$container.parents('.usb-panel-fieldset').length>0;self.isSortable=self.$container.hasClass('sortable');self.isAccordion=self.$container.hasClass('type_accordion');self.isButtonPreview=self.$container.hasClass('preview_button');self.isFieldPreview=self.$container.hasClass('preview_input_fields');self.isCustomColors=self.$container.hasClass('for_custom_colors');self.checkParamForDataIndicator=self.$container.hasClass('check_param_for_data_indicator');var $translations=$('.usof-form-group-translations',container);self.groupTranslations=($translations.length?($translations[0].onclick()||{}):{});if(self.$container.is('[data-filters]')){self._filters=$ush.toPlainObject(self.$container.data('filters'));self.$container.removeAttr('data-filters')}
...
self.hasStringValue=self.$container.closest('.usb-panel-fieldset').length>0;self._std=self.getValue();self.$btnAddGroup.off('click').on('click',self.addGroup.bind(self,_undefined));self.$container.on('change',()=>{self.trigger('change',self)}).on('click','.ui-icon_duplicate',self.duplicateGroup.bind(self)).on('click','.usof-form-group-item-controls > .ui-icon_delete',(e)=>{e.stopPropagation();self.deleteGroup($(e.target).closest('.usof-form-group-item'))});if(self.isAccordion){self.$sections=$('.usof-form-group-item',container);self.$container.on('click','.usof-form-group-item-title',(e)=>{if($ush.toLowerCase(e.target.tagName)!=='div'){return}
self.$sections=$('.usof-form-group-item',container);const $parentSection=$(e.target).closest('.usof-form-group-item');if($parentSection.hasClass('active')){$parentSection.removeClass('active').children('.usof-form-group-item-content').slideUp()}else{$parentSection.addClass('active').children('.usof-form-group-item-content').slideDown()}})}
if(self.isSortable){self.$body=$(document.body);self.$window=$(window);self.$dragshadow=$('<div class="us-bld-editor-dragshadow"></div>');$.extend(self._events,{maybeDragMove:self.maybeDragMove.bind(self),dragMove:self.dragMove.bind(self),dragEnd:self.dragEnd.bind(self)});self.$container.on('dragstart',(e)=>{e.preventDefault()}).on('mousedown','.ui-icon_move',self._dragStart.bind(self))}},_hasClass:function(node,className){return(' '+node.className+' ').indexOf(' '+className+' ')>-1},_isShadow:function(node){return this._hasClass(node,'usof-form-group-dragshadow')},_isSortable:function(node){return this._hasClass(node,'usof-form-group-item')},_changeGroupParam:function(){this.trigger('change',this)},_applyFiltersToParam:function(usofField){const self=this;const name=usofField.name;if(!self._filters[name]){return}
const filters=$ush.toPlainObject(self._filters[name]);const value=usofField.getValue();var newValue=$ush.toString(value);['sanitize_color_slug','unique_value',].map((handler)=>{if(newValue&&filters[handler]&&typeof _filtersHandler[handler]==='function'){newValue=_filtersHandler[handler].call(self,newValue,filters[handler],usofField)}});if(newValue!==value){usofField.setValue(newValue)}},_reInitGroupParams:function(){const self=this;self.groupParams=[];$('.usof-form-group-item',self.$container).each((i,groupParams)=>{const $groupParams=$(groupParams);if($groupParams.closest('.usof-form-group-prototype').length){return}
...
for(const k in groupParams.fields){const field=groupParams.fields[k];field.off('change',self._events.changeGroupParam).on('change',self._events.changeGroupParam);if(!$.isEmptyObject(self._filters[k])){field.off('blur',self._events.applyFiltersToParam).on('blur',self._events.applyFiltersToParam)}}
self.groupParams.push(groupParams)})},_reInitValuesChanged:function(){const self=this;if(!self.isBuilder){if($.isEmptyObject($usof.instance.valuesChanged)){clearTimeout($usof.instance.saveStateTimer);$usof.instance.$saveControl.usMod('status','notsaved')}
const value=self.getValue();$usof.instance.valuesChanged[self.groupName]=value;self.$container.trigger('change',value)}},hasValue:function(){return $ush.checksum(this.getValue())!==$ush.checksum(this.getDefaultValue())},getDefaultValue:function(){return $ush.isUndefined(this._std)?'':this._std},getCurrentValue:function(){const self=this;var result=[];for(const i in self.groupParams){result.push(self.groupParams[i].getCurrentValues())}
if(self.hasStringValue){try{result=$ush.toString(result)}catch(err){console.error(result,err);result=''}}
return result},setValue:function(value){const self=this;if(typeof value==='string'&&self.hasStringValue){try{value=JSON.parse($ush.rawurldecode(value)||'[]')}catch(err){console.error(value,err);value=[]}}
self.groupParams=[];$('.usof-form-group-item',self.$container).each((i,groupParams)=>{const $groupParams=$(groupParams);if(!$groupParams.parent().hasClass('usof-form-group-prototype')){$groupParams.remove()}});$.each(value,(index,paramsValues)=>{const $groupParams=$(self.$prototype.html());if(self.$btnAddGroup.length){self.$btnAddGroup.before($groupParams)}else{self.$container.append($groupParams)}
const groupParams=new $usof.GroupParams($groupParams);groupParams.setValues(paramsValues,!0);for(const k in groupParams.fields){if(!groupParams.fields.hasOwnProperty(k)){continue}
groupParams.fields[k].trigger('change');break}
if(self.isButtonPreview||self.isFieldPreview){const uniqueClass=String($('[data-preview-class-format]',$groupParams).data('preview-class-format'));$('.usof-preview-class-main',$groupParams).text(uniqueClass.replace('%s',paramsValues.id))}});self._reInitGroupParams();self._reInitValuesChanged()},getValue:function(){const self=this;var result=[];$.each(self.groupParams,(_,groupParams)=>{result.push(groupParams.getValues())});if(self.hasStringValue){if(result.length){try{result=$ush.toString(result)}catch(err){console.error(result,err);result=self.getDefaultValue()}}else{result=self.getDefaultValue()}}
return result},addGroup:function(index){const self=this;self.$btnAddGroup.addClass('adding');var $groupPrototype=$(self.$prototype.html());if((self.isButtonPreview||self.isFieldPreview)&&!$ush.isUndefined(index)){self.$btnAddGroup.closest('.usof-form-group').find(` > .usof-form-group-item:eq(${parseInt( index )})`).after($groupPrototype)}else{self.$btnAddGroup.before($groupPrototype)}
const groupParams=new $usof.GroupParams($groupPrototype);for(const k in groupParams.fields){const field=groupParams.fields[k];field.on('change',self._events.changeGroupParam);if(!$.isEmptyObject(self._filters[k])){field.on('blur',self._events.applyFiltersToParam).trigger('blur')}}
if((self.isButtonPreview||self.isFieldPreview)&&index!==_undefined){self.groupParams.splice(index+1,0,groupParams)}else{self.groupParams.push(groupParams)}
...
const value=self.getValue();$usof.instance.valuesChanged[self.groupName]=value;self.$container.trigger('change',value)}
if(self.isButtonPreview||self.isFieldPreview){var newIndex=self.groupParams.length,newId=1,newIndexIsUnique;for(const i in self.groupParams){newId=Math.max((parseInt(self.groupParams[i].fields.id.getValue())||0)+1,newId)}
do{newIndexIsUnique=!0;for(const i in self.groupParams){if(self.groupParams[i].fields.name.getValue()==self.groupTranslations.style+' '+newIndex){newIndex ++;newIndexIsUnique=!1;break}}}while(!newIndexIsUnique);groupParams.fields.name.setValue(self.groupTranslations.style+' '+newIndex);groupParams.fields.id.setValue(newId);const uniqueClass=String($('[data-preview-class-format]',groupParams.$container).data('preview-class-format'));$('.usof-preview-class-main',groupParams.$container).text(uniqueClass.replace('%s',newId))}
if(self.isLiveBuilder){groupParams.setAccordionTitle()}
self.$btnAddGroup.removeClass('adding');return groupParams},duplicateGroup:function(e){const self=this;const $target=$(e.currentTarget);const $group=$target.closest('.usof-form-group-item');const index=$group.index()-1;if(self.groupParams.hasOwnProperty(index)){var $item=self.groupParams[index],values=$item.getValues(),number=0;values.name=values.name.replace(/\s?\(.*\)$/,'').trim();for(const i in self.groupParams){const name=self.groupParams[i].getValue('name')||'';const numMatches=name.match(new RegExp(values.name+'\\s?\\((\\d+)*','m'));if(numMatches!==null){number=Math.max(number,parseInt(numMatches[1]||1))}}
values.name+=' ('+(++ number)+')';const newGroup=self.addGroup(index);newGroup.setValues($.extend(values,{id:newGroup.getValue('id')}))}},deleteGroup:function($group){$group.remove();this._reInitGroupParams();this._reInitValuesChanged()},_dragStart:function(e){const self=this;e.stopPropagation();self.$draggedElm=$(e.target).closest('.usof-form-group-item');self.detached=!1;self._updateBlindSpot(e);self.elmPointerOffset=[e.pageX,e.pageY].map($ush.parseInt);self.$body.on('mousemove',self._events.maybeDragMove);self.$window.on('mouseup',self._events.dragEnd)},_updateBlindSpot:function(e){this.blindSpot=[e.pageX,e.pageY].map($ush.parseInt)},_isInBlindSpot:function(e){const self=this;return(Math.abs(e.pageX-self.blindSpot[0])<=20&&Math.abs(e.pageY-self.blindSpot[1])<=20)},maybeDragMove:function(e){const self=this;e.stopPropagation();if(self._isInBlindSpot(e)){return}
self.$body.off('mousemove',self._events.maybeDragMove);self._detach();self.$body.on('mousemove',self._events.dragMove)},_detach:function(e){const self=this;const offset=self.$draggedElm.offset();self.elmPointerOffset[0]-=offset.left;self.elmPointerOffset[1]-=offset.top;$('.usof-form-group-item-title',self.$draggedElm).hide();if(!self.isAccordion||self.$draggedElm.hasClass('active')){$('.usof-form-group-item-content',self.$draggedElm).hide()}
...
self.$container.addClass('dragging');self.detached=!0},dragMove:function(e){const self=this;e.stopPropagation();self.$draggedElm.css({left:e.pageX-self.elmPointerOffset[0],top:e.pageY-self.elmPointerOffset[1]});if(self._isInBlindSpot(e)){return}
var element=e.target;for(var level=0;level<=2;level ++,element=element.parentNode){if(self._isShadow(element)){return}
if(self._isSortable(element)){var nextElement=element.previousSibling,shadowAtLeft=!1;while(nextElement){if(nextElement==this.$dragshadow[0]){shadowAtLeft=!0;break}
nextElement=nextElement.previousSibling}
self.$dragshadow[shadowAtLeft?'insertAfter':'insertBefore'](element);self._dragDrop(e);break}}},_dragDrop:function(e){this._updateBlindSpot(e)},dragEnd:function(e){const self=this;self.$body.off('mousemove',self._events.maybeDragMove).off('mousemove',self._events.dragMove);self.$window.off('mouseup',self._events.dragEnd);if(self.detached){self.$draggedElm.removeClass('dragged').removeAttr('style').insertBefore(self.$dragshadow);self.$dragshadow.detach();if(self.isBuilder){self.$builderWindow.removeClass('dragged')}
...
self.$container.removeClass('dragging')}});$.fn.usofGroup=function(options){return new $usof.Group(this,options)}}(jQuery);!function($,_undefined){"use strict";$usof.GroupParams=function(container){const self=this;self.$container=$(container);self.$group=self.$container.closest('.usof-form-group');self.group=self.$group.data('name');self.isGroupParams=!0;self.isBuilder=self.$container.parents('.us-bld-window').length>0;self.isButtonPreview=self.$group.hasClass('preview_button');self.isFieldPreview=self.$group.hasClass('preview_input_fields');self._events={setAccordionTitle:self.setAccordionTitle.bind(self),};self.initFields(self.$container);self.fireFieldEvent(self.$container,'beforeShow');self.fireFieldEvent(self.$container,'afterShow');var accordionTitle=self.$group.data('accordion-title');if(!$ush.isUndefined(accordionTitle)){accordionTitle=decodeURIComponent(accordionTitle)}
self.accordionTitle=accordionTitle;if(self.hasAccordionTitle()){for(const fieldId in self.fields){if(!self.fields.hasOwnProperty(fieldId)){continue}
self.fields[fieldId].on('change',self._events.setAccordionTitle)}}
if(self.isButtonPreview||self.isFieldPreview){for(const fieldId in self.fields){if(fieldId!=='class'&&self.fields.hasOwnProperty(fieldId)){continue}
...
self.fields[fieldId].on('change',()=>{if($usof.instance&&$.isEmptyObject($usof.instance.valuesChanged)){clearTimeout($usof.instance.saveStateTimer);$usof.instance.$saveControl.usMod('status','notsaved');}
...
self.$container.data('usof.GroupParams',self);if(self.isButtonPreview){$('.usof-btn-preview',self.$container).USOF_ButtonPreview()}else if(self.isFieldPreview){$('.usof-input-preview',self.$container).USOF_FieldPreview()}};$.extend($usof.GroupParams.prototype,$usof.mixins.Fieldset,{hasAccordionTitle:function(){return $ush.toString(this.accordionTitle)!==''},setAccordionTitle:function(){const self=this;if(!self.hasAccordionTitle()){return}
self.$title=$('.usof-form-group-item-title',self.$container);if(self.isButtonPreview){self.$title=$('.usof-btn-label',self.$title)}
if(self.isFieldPreview){self.$title=$('input.usof-input-preview-elm',self.$title)}
var title=self.accordionTitle;for(const fieldId in self.fields){if(!self.fields.hasOwnProperty(fieldId)||title.indexOf(fieldId)<0){continue}
const field=self.fields[fieldId];var value=self.getValue(fieldId);if(field.hasOwnProperty('type')&&field.type==='select'){var $option=$(`option[value="${value}"]`,field.$container);if($option.length&&$option.html()!==''){value=$option.html()}}
title=title.replace(fieldId,value)}
if(self.isFieldPreview){self.$title.attr('placeholder',title)}else{self.$title.text(title)}}})}(jQuery);!function($,_undefined){"use strict";$usof.Meta=function(container){const self=this;window.USMMSettings=window.USMMSettings||{};self.$container=$(container);self.initFields(self.$container);self.fireFieldEvent(self.$container,'beforeShow');self.fireFieldEvent(self.$container,'afterShow');for(const fieldId in self.fields){if(!self.fields.hasOwnProperty(fieldId)){continue}
self.fields[fieldId].on('change',(field,value)=>{for(const savingFieldId in self.fields){window.USMMSettings[savingFieldId]=self.fields[savingFieldId].getValue()}
$(document.body).trigger('usof_mm_save');})}
self.$container.data('usof.Meta',self)};$.extend($usof.Meta.prototype,$usof.mixins.Fieldset);$(()=>{$.each($('.usof-container.for_meta'),(_,node)=>{new $usof.Meta(node)});$(document.body).off('usof_mm_load').on('usof_mm_load',()=>{$('.us-mm-settings').each((_,node)=>{new $usof.Meta(node)})})})}(jQuery);!function($,_undefined){"use strict";const max=Math.max;const JSON_MIME_TYPE='application/json';$usof.Form=function(container){const self=this;self.$window=$(window);self.$container=$(container);if(self.$container.length===0){return}
self.$header=$('.usof-header',self.$container);self.$title=$('.usof-header-title h2',self.$container);self.$controlForSchemes=$('.for_color_schemes',self.$container);self.$controlForImportExport=$('.for_import_export',self.$container);self.$container.addClass('inited');self.active=null;self.$sections={};self.$sectionContents={};self.sectionFields={};self.isThemeOptionsPage=self.$container.hasClass('theme_options_page');$usof.instance=self;self.initFields(self.$container);self.initDataIndicator();self._events={scroll:self.scroll.bind(self),resize:self.resize.bind(self),onImportStyles:self.onImportStyles.bind(self),onExportStyles:self.onExportStyles.bind(self),disableExportStyles:$ush.debounce(self.disableExportStyles.bind(self),2),};$('.usof-section',self.$container).each((index,section)=>{var $section=$(section),sectionId=$section.data('id');self.$sections[sectionId]=$section;self.$sectionContents[sectionId]=$('.usof-section-content',$section);if($section.hasClass('current')){self.active=sectionId}
self.sectionFields[sectionId]=[];$('.usof-form-row',$section).each((index,row)=>{const fieldName=$(row).data('name');if(fieldName){self.sectionFields[sectionId].push(fieldName)}})});self.sectionTitles={};$('.usof-nav-item.level_1',self.$container).each((index,item)=>{const $item=$(item);self.sectionTitles[$item.data('id')]=$('.usof-nav-title',$item).html()});self.navItems=$('.usof-nav-item.level_1, .usof-section-header',self.$container);$('.usof-section-header',self.$container).each((_,item)=>{const $item=$(item);$item.on('click',()=>self.openSection($item.data('id')))});if(!$ush.isUndefined(self.$sections[self.active])){self.fireFieldEvent(self.$sections[self.active],'beforeShow');self.fireFieldEvent(self.$sections[self.active],'afterShow')}
self.$saveControl=$('.usof-control.for_save',self.$container);self.$saveBtn=$('.usof-button',self.$saveControl).on('click',self.saveChanges.bind(self));self.$saveMessage=$('.usof-control-message',self.$saveControl);self.valuesChanged={};self.saveStateTimer=null;for(const fieldId in self.fields){if(!self.fields.hasOwnProperty(fieldId)){continue}
self.fields[fieldId].on('change',(field,value)=>{if($.isEmptyObject(self.valuesChanged)){clearTimeout(self.saveStateTimer);self.$saveControl.usMod('status','notsaved');}
self.valuesChanged[field.name]=value})}
self.resize();if(self.isThemeOptionsPage){self.$importStyles=$('.import_styles',self.$controlForImportExport);self.$exportStyles=$('.export_styles',self.$controlForImportExport);if(self.groups.buttons instanceof $usof.Group){self.groups.buttons.on('change',self._events.disableExportStyles)}
if(self.groups.input_fields instanceof $usof.Group){self.groups.input_fields.on('change',self._events.disableExportStyles)}
self.$controlForSchemes.on('click',()=>$('.usof-form-row.type_style_scheme').show());self.$controlForImportExport.on('click','.export_styles',self._events.onExportStyles).on('click','.import_styles',self._events.onImportStyles)}
self.$window.on('resize load',self._events.resize).on('scroll',self._events.scroll).on('hashchange',()=>self.openSection(document.location.hash.substring(1)));self.$window.on('keydown',(event)=>{if(event.ctrlKey||event.metaKey){if(String.fromCharCode(event.which).toLowerCase()=='s'){event.preventDefault();$usof.instance.saveChanges()}}});if(document.location.hash&&document.location.hash.indexOf('#!')==-1){self.openSection(document.location.hash.substring(1))}
self.$container.data('usof.Form',self)};$.extend($usof.Form.prototype,$usof.mixins.Fieldset,{openSection:function(sectionId){const self=this;if(sectionId==self.active||$ush.isUndefined(self.$sections[sectionId])){return}
if(!$ush.isUndefined(self.$sections[self.active])){self.hideSection()}
self.$controlForSchemes.toggleClass('hidden',sectionId!=='colors');self.$controlForImportExport.toggleClass('hidden',!['buttons','input_fields','typography'].includes(sectionId));self.showSection(sectionId)},showSection:function(sectionId){const self=this;const $curItem=self.navItems.filter(`[data-id="${sectionId}"]`);$curItem.addClass('current');self.fireFieldEvent(self.$sectionContents[sectionId],'beforeShow');self.$sectionContents[sectionId].stop(!0,!1).fadeIn();self.$title.html(self.sectionTitles[sectionId]);self.fireFieldEvent(self.$sectionContents[sectionId],'afterShow');const itemPopup=$('.usof-nav-popup',$curItem);if(itemPopup.length>0){const matches=document.cookie.match(/(?:^|; )usof_visited_new_sections=([^;]*)/);const cookieValue=matches?decodeURIComponent(matches[1]):'';const visitedNewSections=(cookieValue=='')?[]:cookieValue.split(',');if(visitedNewSections.indexOf(sectionId)==-1){visitedNewSections.push(sectionId);document.cookie='usof_visited_new_sections='+visitedNewSections.join(',')}
itemPopup.remove()}
self.active=sectionId},hideSection:function(){const self=this;self.navItems.filter(`[data-id="${self.active}"]`).removeClass('current');self.fireFieldEvent(self.$sectionContents[self.active],'beforeHide');self.$sectionContents[self.active].stop(!0,!1).hide();self.$title.html('');self.fireFieldEvent(self.$sectionContents[self.active],'afterHide');self.active=null},onExportStyles:function(){const self=this;const sectionId=document.location.hash.substring(1);const allValues=self.getValues();var name=sectionId;if(sectionId==='buttons'){name='button_styles'}
if(sectionId==='input_fields'){name='field_styles'}
var data={};if(sectionId==='typography'){const typographyKeys=['h1','h2','h3','h4','h5','h6','body','adobe_fonts','custom_font','font-family','font-size','font-style','font-weight','font-display','uploaded_fonts'];typographyKeys.forEach(key=>{if(!$ush.isUndefined(allValues[key])&&key!=='adobe_fonts'){data[key]=allValues[key]}
if(key==='adobe_fonts'){const typekitId=$('[name="typekit_id"]').val();if(typekitId){data[key]=typekitId}}});data=JSON.stringify(data)}else{data=JSON.stringify((allValues||{})[sectionId]??{})}
if(data){$ush.download(data,`${ name }_[%s].json`.replace('%s',window.location.host),JSON_MIME_TYPE)}},onImportStyles:function(){const self=this;const fileInput=document.createElement('input');const ERROR_INCORRECT_FILE='Incorrect file';const sectionId=document.location.hash.substring(1);const loadStyles=(e)=>{try{const fileData=JSON.parse(String(e.target.result).trim());if(sectionId==='typography'){if(!fileData||$ush.isUndefined((fileData||{})['font-family'])){throw new Error()}
if(!confirm('Override the current Typography settings?')){return}
const allValues=self.getValues()||{};Object.keys(fileData).forEach((key)=>{allValues[key]=fileData[key]});self.setValues(allValues);$(window).trigger('usof_typography_import');if(fileData.adobe_fonts){$('[name="typekit_id"]').val(fileData.adobe_fonts).trigger('change');$('.usof-button.type_adobe_fonts_apply').trigger('click')}}else if(self.groups[sectionId]instanceof $usof.Group){if(!Array.isArray(fileData)||fileData.length===0){throw new Error}
if(sectionId==='buttons'&&$ush.isUndefined(fileData[0].hover_text_animation)){throw new Error}
if(sectionId==='input_fields'&&$ush.isUndefined(fileData[0].checkbox_size)){throw new Error}
var maxId=0,values=$ush.toArray(self.groups[sectionId].getValue());values.map((style)=>{maxId=max(maxId,$ush.parseInt(style.id))});if(maxId){fileData.map((item)=>{item.id=++maxId})}
self.groups[sectionId].setValue(values.concat(fileData))}}catch(err){return alert(ERROR_INCORRECT_FILE)}};fileInput.type='file';fileInput.accept=JSON_MIME_TYPE;fileInput.onchange=()=>{const stylesFile=fileInput.files[0];if(!stylesFile||stylesFile.type!==JSON_MIME_TYPE){return alert(ERROR_INCORRECT_FILE)}
self.$importStyles.addClass('disabled');const reader=new FileReader;reader.addEventListener('load',loadStyles);reader.addEventListener('loadend',()=>{$ush.timeout(()=>{self.$importStyles.removeClass('disabled')},5)});reader.addEventListener('error',()=>{alert('Error reading the file. Please try again.')});reader.readAsText(stylesFile)};fileInput.click()},disableExportStyles:function(theGroup){const self=this;var value=theGroup.getValue();if(!Array.isArray(value)){value=[]}
self.$exportStyles.toggleClass('disabled',value.length===0)},saveChanges:function(){const self=this;if($.isEmptyObject(self.valuesChanged)){return}
clearTimeout(self.saveStateTimer);self.$saveMessage.html('');self.$saveControl.usMod('status','loading');$.ajax({type:'POST',url:$usof.ajaxUrl,dataType:'json',data:{action:'usof_save',usof_options:JSON.stringify(self.valuesChanged),_wpnonce:$('[name="_wpnonce"]',self.$container).val(),_wp_http_referer:$('[name="_wp_http_referer"]',self.$container).val()},success:(result)=>{if(result.success){self.valuesChanged={};self.$saveMessage.html(result.data.message);self.$saveControl.usMod('status','success');self.saveStateTimer=setTimeout(()=>{self.$saveMessage.html('');self.$saveControl.usMod('status','clear')},4000)}else{self.$saveMessage.html(result.data.message);self.$saveControl.usMod('status','error');self.saveStateTimer=setTimeout(()=>{self.$saveMessage.html('');self.$saveControl.usMod('status','notsaved')},4000)}}})},scroll:function(){this.$container.toggleClass('footer_fixed',this.$window.scrollTop()>this.headerAreaSize)},resize:function(){const self=this;if(!self.$header.length){return}
self.headerAreaSize=self.$header.offset().top+self.$header.outerHeight();self.scroll()},});$(()=>new $usof.Form('.usof-container:not(.inited)'))}(jQuery);!function($,_undefined){"use strict";$usof.dragDrop=function(container,itemSelector,checkDraggable){let self=this;self.itemSelector=itemSelector;self.checkDraggable=checkDraggable;self.isPlaceAfter=!1;self._events={dragstart:self._dragstart.bind(self),dragover:self._dragover.bind(self),drop:self._drop.bind(self),dragend:self._dragend.bind(self),};self.$container=$(container);self.init()};$.extend($usof.dragDrop.prototype,$usof.mixins.Events,{init:function(){let self=this;self.$container.addClass('usof-dragdrop').data('usof.dragDrop',self).on('mousedown',self.itemSelector,function(e){if(self.checkDraggable&&!e.target.className.includes('usof_draggable')){return!0}
...
let value=self.getCurrentValue(),isVC=self.isWPBakeryParamValue(),isNumericValue=$ush.parseInt(value)!==0,$addDesc=$('.us-grid-layout-desc-add',isVC?self.$row.parent():self.$row),$editLink=$('.us-grid-layout-desc-edit',isVC?self.$row.parent():self.$row);if(isNumericValue){$('.edit-link',$editLink).attr('href',(self.$container.data('edit_link')||'').replace('%d',value))}
...
$node=$(node);$node.data('text',$node.text());self._$fontWeights.push($node)})}}});self.$row.on('click','> .usof-form-row-title',self._events.switchSection);if(!self.isLiveBuilder()){self.on('afterShow',()=>{self._setFontProperties(self.getValue())})}
$(window).on('usof_typography_import',()=>{self._renderFontPreview()})},_switchSection:function(e){var self=this;self.$row.toggleClass('expand',!self.$row.hasClass('expand')).siblings().removeClass('expand')},_syncAvailableWeights:function(fontFamily){var self=this;for(var i in self._$fontWeights){var $node=self._$fontWeights[i];if($node.hasClass('not-exists')){$node.text($node.data('text')).removeClass('not-exists')}}
...
if($.isPlainObject(parentValues.h1)){['font-family','font-weight','bold-font-weight','font-style','text-transform'].map(function(name){if(!$ush.isUndefined(parentValues.h1[name])){var value=parentValues.h1[name];if(name==='font-family'){value=_getFontFamily(value)}
...
if(self.isMultiple()&&self.options.sortable){const dragdrop=new $usof.dragDrop(self.$options,'> .usof-autocomplete-selected')
dragdrop.on('changed',self._saveValue.bind(self))}},isMultiple:function(){return!!this.options.multiple},getItems:function(callback,itemIds){const self=this;if(!$ush.isUndefined(_$temp.xhr)&&!_$temp.xhr._skipAbort){_$temp.xhr.abort()}
...
self.clearMessage();self.$container.addClass('loading');var valueList=$ush.toString(self.getValue()).split(valueSeparator);_$temp.xhr=$.ajax({data:data,dataType:'json',url:$usof.ajaxUrl,cache:!1,success:(res)=>{if(!res.success){self._showMessage.call(self,res.data.message);return}
var items=res.data.items;if(!Array.isArray(items)){items=[]}
if(!itemIds.length){items.map((item)=>{var itemValue=$ush.toString(item.value);var $item=$('<div>',{'data-value':$ush.stripTags(itemValue),'data-text':$ush.stripTags(item.name),'tabindex':3});if(valueList.indexOf(itemValue)>-1){$item.addClass('selected')}
if($(`[data-value="${itemValue}"]`,self.$list).length===0||itemIds.length>0){self.$list.append($item.html(item.name))}})}
if(typeof callback==='function'){callback.call(self,items)}
self.trigger('data.loaded',res.data.items)},complete:(_,textStatus)=>{if(textStatus!=='abort'){_$temp.xhr=_undefined;self.$container.removeClass('loading')}}});if(self.$list.is(':empty')){_$temp.xhr._skipAbort=!0}else{_$temp.xhr._skipAbort=itemIds.length}},_searchRequest:function(e){const self=this;if(_$temp.typingTimer){$ush.clearTimeout(_$temp.typingTimer)}
_$temp.typingTimer=$ush.timeout(()=>{self._clearList();self.getItems((items)=>{if($.isEmptyObject(items)){self.showMessage(self.options.no_results_found)}});$ush.clearTimeout(_$temp.typingTimer)},500)},_searchItems:function(e){const self=this;var value=$ush.toLowerCase(e.currentTarget.value);self.clearMessage();if(!value){$('.hidden',self.$list).removeClass('hidden');return}
var $items=$('[data-value]',self.$list);$items.addClass('hidden').filter(`[data-text^="${value}"], [data-text*="${value}"]`).removeClass('hidden');$('[data-group]',self.$list).each((_,node)=>{$(node).toggleClass('hidden',!$('[data-value]:not(.hidden)',node).length)});if(!$items.is(':visible')){self.showMessage(self.options.no_results_found)}},_keyup:function(e){const self=this;if(e.keyCode===$ush.ENTER_KEYCODE){$('[data-value]:not(.selected):visible:first',self.$list).trigger('click')}
if([$ush.ENTER_KEYCODE,$ush.ESC_KEYCODE].includes(e.keyCode)){self.$toggle.removeClass('show');self.$search.trigger('blur')}},_showList:function(e){const self=this;self.$toggle.addClass('show');self.$search[0].focus()},_toggleList:function(e){const self=this;var isFocus=(e.type==='focus'),handle=$ush.timeout(()=>{self.$toggle.toggleClass('show',isFocus);$ush.clearTimeout(handle)},(isFocus?0:150))},_hideList:function(e){const self=this;if(self.isMultiple()&&self.$toggle.hasClass('show')){self.$toggle.removeClass('show');self.$search.blur()}},_scrollList:function(e){const self=this;var target=e.currentTarget,$target=$(target);if(!_$temp.endOfListReached&&$ush.isUndefined(_$temp.xhr)&&($target.scrollTop()+$target.height())>=(target.scrollHeight-1)){self.getItems((items)=>{if($.isEmptyObject(items)){_$temp.endOfListReached=!0}})}},_clearList:function(){_$temp.endOfListReached=!1;$('[data-value]',this.$list).remove()},_loadSelectedValues:function(value){const self=this;var $selected=$('.usof-autocomplete-selected',self.$options),textInPlaceholder=self.options.text_in_placeholder;if(!self.hasAjaxHandler||$selected.length){return}
$('.selected',self.$list).removeClass('selected');$selected.remove();if(value){var itemIds=value.split(self.options.value_separator).map($.trim);self.$options.addClass('show_placeholders');itemIds.map((id)=>{self._showSelectedValue(id,textInPlaceholder)});self.getItems((items)=>{$('.usof-autocomplete-selected',self.$options).remove();self.$options.removeClass('show_placeholders');var itemLabels={};items.map((item)=>{itemLabels[item.value]=item.name});itemIds.map((id)=>{self._showSelectedValue(id,itemLabels[id]||id)})},itemIds)}},_showSelectedValues:function(value){const self=this;var $selected=$('.usof-autocomplete-selected',self.$options);$('.selected',self.$list).removeClass('selected');$selected.remove();if(value){var itemIds=value.split(self.options.value_separator);itemIds.map((itemId)=>{self._showSelectedValue(itemId,$(`[data-value="${itemId}"]:first`,self.$list).text())})}},_showSelectedValue:function(value,label){const self=this;if(!value){return}
...
self.$options.append(`<div class="usof-autocomplete-selected">
				...
				</div>`);$(`[data-value="${value}"]:not(.selected)`,self.$list).addClass('selected')},_selectedValue:function(e){const self=this;var $target=$(e.currentTarget);if($target.hasClass('selected')&&self.isMultiple()){self._removeSelectedValue(e);return}
if(!self.isMultiple()){$('[data-value]',self.$list).removeClass('selected');$('.usof-autocomplete-selected',self.$options).remove()}
var value=$target.data('value');self._showSelectedValue(value,$target.html());self._saveValue()},_removeSelectedValue:function(e){const self=this;const value=$(e.currentTarget).data('value');$(`[data-value="${value}"]`,self.$list).removeClass('selected');$(`[data-value="${value}"]`,self.$options).parent().remove();self._saveValue()},_saveValue:function(){const self=this;var value=[];$('[data-value]',self.$options).each((_,node)=>{value.push($(node).data('value'))});self.setValue(value.join(self.options.value_separator))},_changeRelatedField:$ush.debounce(function(){const self=this;if($ush.isUndefined(_$temp.xhr)){$('.usof-autocomplete-selected',self.$options).remove();$('.selected',self.$list).removeClass('selected');self.$value.val('');self.trigger('change')}
self._clearList();self.getItems((items)=>{if($.isEmptyObject(items)){self.showMessage(self.options.no_results_found)}})},1),showMessage:function(text){const self=this;self.$list.addClass('hidden');self.$message.text(text).removeClass('hidden')},clearMessage:function(){const self=this;self.$list.removeClass('hidden');self.$message.addClass('hidden').text('')},getValue:function(){const self=this;if(self.$value instanceof $){return self.$value.val()}else{var $value=$('.usof-autocomplete-value',self.$row)
return($value.length)?$value.val():''}},setValue:function(value,quiet){const self=this;self.$value.val(value);if(self.hasAjaxHandler){if(value){self._loadSelectedValues(value)}}else{self._showSelectedValues(value)}
...
this.$preview.attr('class',icon_set+' fa-'+icon_name).html('')}}else{this.$preview.attr('class','material-icons').html('')}},setValue:function(value,quiet){this.renderValue(value);this.parentSetValue(value,quiet)}}}(jQuery);!function($,_undefined){const _document=document;const _window=window;const abs=Math.abs;const max=Math.max;const min=Math.min;const round=Math.round;if($ush.isUndefined(_window.$usof)){return}
var oldCurrentValue;const _mouseEvents=['wheel','mousewheel','DOMMouseScroll'];var _dragData={};$usof.field.slider={init:function(options){const self=this;const _defaultRangeSettings={min:0,max:60,step:1};self._unitsSettings={};self.isFocused=!1;self.$window=$(_window);self.$body=$(_document.body);self.$usofContainer=$('.usof-container');self.$slider=$('.usof-slider',self.$row);self.$box=$('.usof-slider-box',self.$slider);self.$range=$('.usof-slider-range',self.$slider);self.$units=$('.usof-slider-selector-unit',self.$slider);self.$unitSelector=$('.usof-slider-selector input',self.$slider);if(self.$slider.is('[onclick]')){$.each(self.$slider[0].onclick()||{},(unit,options)=>{options=$.extend({},_defaultRangeSettings,options);self._unitsSettings[unit]=options});self.$slider.removeAttr('onclick')}
if($.isEmptyObject(self._unitsSettings)){self._unitsSettings.px=$ush.clone(_defaultRangeSettings)}
self._events={setResponsiveScreen:$ush.debounce(self._setResponsiveScreen.bind(self),1),selectedUnit:self._selectedUnit.bind(self),blur:self._blur.bind(self),focus:self._focus.bind(self),keyup:self._keyup.bind(self),mouseenter:self._mouseenter.bind(self),mouseleave:self._mouseleave.bind(self),mousewheel:self._mousewheel.bind(self),dragmove:self._dragmove.bind(self),dragstart:self._dragstart.bind(self),dragstop:self._dragstop.bind(self),};self.$slider.on('mousedown','.usof-slider-box',self._events.dragstart).on('mousedown','.usof-slider-selector-unit',self._events.selectedUnit);self.$unitSelector.on('mouseenter',self._events.mouseenter).on('mouseleave',self._events.mouseleave).on('keyup',self._events.keyup).on('focus',self._events.focus).on('blur',self._events.blur);if(self.hasResponsive()){self.on('setResponsiveState',self._events.setResponsiveScreen);if(self.$row.closest('.theme_options_page').length){$ush.timeout(()=>self.setValue(self.getValue(),!0),1)}}},getUnitData:function(value){const self=this;var unitsSettings=self._unitsSettings;value+='';var unit=(value.match(/^((-?\d+)(\.)?(\d+)?)([a-z\%]+)?$/)||[])[5]||'';if($ush.isUndefined(unitsSettings[unit])){unit=$ush.toString(Object.keys(unitsSettings)[0])}
...
return decimalValue.toFixed(0)},renderValue:function(value){const self=this;var unitData=self.getUnitData(value),unit=unitData.unit,value=unitData.value;self.$unitSelector.val(value+unit);self.$units.removeClass('active').filter('[data-unit="'+unit+'"]').addClass('active');var unitSettings=self._unitsSettings[unit],offset=max(0,min(1,(value-unitSettings.min)/(unitSettings.max-unitSettings.min)));self.$range.css(($ush.isRtl()?'right':'left'),offset*100+'%')},_setResponsiveScreen:function(e){const self=this;self.renderValue(self.getCurrentValue())},_dragstart:function(e){const self=this;e.stopPropagation();var boxWidth=self.$box.width(),currentValue=self.getCurrentValue(),unit=self.getUnitData(currentValue).unit,boxOffsetLeft=self.$box.offset().left;if(self.$usofContainer.length==0){self.$usofContainer=$('.usof-container')}
self.$usofContainer.addClass('dragged');self.$box.addClass('dragged');_dragData={currentValue:currentValue,left:boxOffsetLeft,right:boxOffsetLeft+boxWidth,unit:unit,unitSettings:self._unitsSettings[unit]||{},width:boxWidth,};self.$body.on('mousemove',self._events.dragmove);self.$window.on('mouseup',self._events.dragstop);self._events.dragmove(e)},_dragmove:function(e){const self=this;e.stopPropagation();var isEmptyDragData=$.isEmptyObject(_dragData),unitSettings=_dragData.unitSettings;var x,offset,value;if($ush.isRtl()){offset=isEmptyDragData?0:(_dragData.right-e.pageX)/_dragData.width}else{offset=isEmptyDragData?0:(e.pageX-_dragData.left)/_dragData.width}
x=max(0,min(1,offset));value=$ush.parseFloat(unitSettings.min+x*(unitSettings.max-unitSettings.min));value=round(value/unitSettings.step)*unitSettings.step;value=self.roundValue(value)+_dragData.unit;self.renderValue(value);self.setCurrentValue(value)},_dragstop:function(e){e.preventDefault();e.stopPropagation();const self=this;if(!self.$usofContainer.hasClass('dragged')){return}
self.$usofContainer.removeClass('dragged');self.$box.removeClass('dragged');self.$body.off('mousemove',self._events.dragmove);self.$window.off('mouseup',self._events.dragstop)},_mouseenter:function(e){const self=this;_mouseEvents.map((eventType)=>{_window.addEventListener(eventType,self._events.mousewheel,{passive:!1})})},_mouseleave:function(e){const self=this;_mouseEvents.map((eventType)=>{_window.removeEventListener(eventType,self._events.mousewheel)})},_mousewheel:function(e){const self=this;e.preventDefault?e.preventDefault():(e.returnValue=!1);e.stopPropagation();if(!self.isFocused){return!1}
...
value=round(value/unitSettings.step)*unitSettings.step;value=self.roundValue(value)+unit;self.renderValue(value);self.setCurrentValue(value)},_keyup:function(e){if($ush.toLowerCase(e.key)==='enter'){this.$unitSelector.trigger('blur')}},_focus:function(e){const self=this;self.isFocused=!0;oldCurrentValue=self.getCurrentValue()},_blur:function(e){const self=this;var value=$ush.toString(self.$unitSelector.val()),unitData=self.getUnitData(value),unitSettings=self._unitsSettings[unitData.unit];self.isFocused=!1;value=unitData.value;if(value===0){value=oldCurrentValue}
value+=''+unitData.unit;self.renderValue(value);self.setCurrentValue(value)},_selectedUnit:function(e){const self=this;var $target=$(e.target),value=self.getUnitData(self.getCurrentValue()).value;value+=$ush.toString($target.data('unit'));self.$units.removeClass('active');$target.addClass('active');self.renderValue(value);self.setCurrentValue(value)},setValue:function(value,quiet){const self=this;if(!self.isResponsiveValue(value)){self.renderValue(value)}else{self.renderValue(self.getCurrentValue())}
self.parentSetValue(''+value,quiet)}}}(jQuery);!function($,_undefined){if($ush.isUndefined(window.$usof)){return}
$usof.field['switch']={init:function(){const self=this;self.$hidden=$('input.wpb_vc_param_value',self.$row);self.$input.on('change',(e)=>{const value=self.getValue();e.target.value=value;if(self.isWPBakeryParamValue()){self.$hidden.val(value).trigger('change')}
self.trigger('change',[value])})},isWPBakeryParamValue:function(){return this.$hidden.length>0},getValue:function(){return this.$input.is(':checked')?1:0},setValue:function(value,quiet){const self=this;value=$ush.parseInt(value);self.$input.val(value).prop('checked',value);if(!quiet){self.trigger('change',[value])}}}}(jQuery);!function($,_undefined){"use strict";if($ush.isUndefined(window.$usof)){return}
$usof.field.text={init:function(){const self=this;self._dynamicLabels={};self.$text=$('input[type=text]',self.$row);self._events={blurField:self._blurField.bind(self),changeField:$ush.debounce(self.changeField.bind(self),1),setExampleValue:self.setExampleValue.bind(self),syncCurrentValue:self.syncCurrentValue.bind(self),};self.initDynamicValue();self.$row.on('click','.usof-example',self._events.setExampleValue).on('change paste keyup','input[type=text]',self._events.changeField).on('blur','input[type=text]',self._events.blurField);if(self.hasResponsive()){self.on('setResponsiveState',self._events.syncCurrentValue)}},setExampleValue:function(e){const self=this;const value=$(e.target).closest('.usof-example').html()||'';self.$text.val(value);self.setCurrentValue(value)},changeField:function(e){this.setCurrentValue(e.target.value)},_blurField:function(e){this.trigger('blur',this.getCurrentValue())},syncCurrentValue:function(){this.$text.val(this.getCurrentValue())},setValue:function(value,quiet){const self=this;self.parentSetValue(value,quiet);self.syncCurrentValue();if(self.popupId){self.setDynamicValue(value)}}};$.extend($usof.field.text,{initDynamicValue:function(){const self=this;$.extend(self._events,{selectDynamicVariable:self.selectDynamicValue.bind(self),removeDynamicValue:self.removeDynamicValue.bind(self),})
...
function us_wpautop(text){var restoreLineBreak=!1,restoreTagBr=!1;const blockTags='table|thead|tfoot|caption|col|colgroup|tbody|tr|td|th|div|dl|dd|dt|ul|ol|li|pre|form|map|area|blockquote|address|math|style|p|h[1-6]|hr|fieldset|legend|section|article|aside|hgroup|header|footer|nav|figure|figcaption|details|menu|summary';text=$ush.toString(text);text=text.replace(/\r\n|\r/g,"\n");if(text.includes('<object')){text=text.replace(/<object[\s\S]+?<\/object>/g,(match)=>{return match.replace(/\n+/g,'')})}
text=text.replace(/<[^<>]+>/g,(tag)=>{return tag.replace(/[\n\t ]+/g,' ')});if(text.includes('<pre')||text.includes('<script')){restoreLineBreak=!0;text=text.replace(/<(pre|script)[^>]*>[\s\S]*?<\/\1>/g,(match)=>{return match.replace(/\n/g,'<wp-line-break>')})}
if(text.includes('<figcaption')){text=text.replace(/\s*(<figcaption[^>]*>)/g,"$1").replace(/<\/figcaption>\s*/g,'</figcaption>')}
if(text.includes('[caption')){restoreTagBr=!0;text=text.replace(/\[caption[\s\S]+?\[\/caption\]/g,(match)=>{return match.replace(/<br([^>]*)>/g,"<wp-temp-br$1>").replace(/<[^<>]+>/g,(tag)=>tag.replace(/[\n\t ]+/,' ')).replace(/\s*\n\s*/g,"<wp-temp-br />")})}
text=(text+"\n\n").replace(/<br \/>\s*<br \/>/gi,"\n\n").replace(new RegExp(`(<(?:${blockTags})(?: [^>]*)?>)`,"gi"),"\n\n$1").replace(new RegExp(`(</(?:${blockTags})>)`,"gi"),"$1\n\n").replace(/<hr( [^>]*)?>/gi,"<hr$1>\n\n").replace(/\s*<option/gi,"<option").replace(/<\/option>\s*/gi,"</option>").replace(/\n\s*\n+/g,"\n\n").replace(/([\s\S]+?)\n\n/g,"<p>$1</p>\n").replace(/<p>\s*?<\/p>/gi,"").replace(new RegExp(`<p>\\s*(</?(?:${blockTags})(?: [^>]*)?>)\\s*</p>`,"gi"),"$1").replace(/<p>(<li.+?)<\/p>/gi,"$1").replace(/<p>\s*<blockquote([^>]*)>/gi,"<blockquote$1><p>").replace(/<\/blockquote>\s*<\/p>/gi,"</p></blockquote>").replace(new RegExp(`<p>\\s*(</?(?:${blockTags})(?: [^>]*)?>)`,"gi"),"$1").replace(new RegExp(`(</?(?:${blockTags})(?: [^>]*)?>)\\s*</p>`,"gi"),"$1").replace(/(<br[^>]*>)\s*\n/gi,"$1").replace(/\s*\n/g,"<br />\n").replace(new RegExp(`(</?(?:${blockTags})[^>]*>)\\s*<br />`,"gi"),"$1").replace(/<br \/>(\s*<\/?(?:p|li|div|dl|dd|dt|th|pre|td|ul|ol)>)/gi,"$1").replace(/(?:<p>|<br ?\/?>)*\s*\[caption([^\[]+)\[\/caption\]\s*(?:<\/p>|<br ?\/?>)*/gi,"[caption$1[/caption]").replace(/(<(?:div|th|td|form|fieldset|dd)[^>]*>)(.*?)<\/p>/g,(m,startTag,content)=>{return content.match(/<p( [^>]*)?>/)?m:`${startTag}<p>${content}</p>`});if(restoreLineBreak){text=text.replace(/<wp-line-break>/g,"\n")}
if(restoreTagBr){text=text.replace(/<wp-temp-br([^>]*)>/g,"<br$1>")}
return text.replace(/<p>(<!--(?:.*)-->)<\/p>/g,"$1")}
function us_wpnop(text){var blockTags,preTags,restoreLineBreak,restoreTagBr,restoreCode;text=$ush.toString(text);if(!text){return ''}
blockTags='blockquote|ul|ol|li|dl|dt|dd|table|thead|tbody|tfoot|tr|th|td|h[1-6]|fieldset|figure|div|p';preTags=blockTags+'|pre';restoreLineBreak=!1;restoreTagBr=!1;restoreCode=[];if(text.includes('<script')||text.includes('<style')){text=text.replace(/<(script|style)[^>]*>[\s\S]*?<\/\1>/g,(match)=>{restoreCode.push(match);return '<wp-preserve>'})}
if(text.includes('<pre')){restoreLineBreak=!0;text=text.replace(/<pre[^>]*>[\s\S]+?<\/pre>/g,(match)=>{return match.replace(/<br ?\/?>(\r\n|\n)?/g,'<wp-line-break>').replace(/<\/?p( [^>]*)?>(\r\n|\n)?/g,'<wp-line-break>').replace(/\r?\n/g,'<wp-line-break>')})}
if(text.includes('[caption')){restoreTagBr=!0;text=text.replace(/\[caption[\s\S]+?\[\/caption\]/g,(match)=>{return match.replace(/<br([^>]*)>/g,"<wp-temp-br$1>").replace(/[\r\n\t]+/,"")})}
text=text.replace(/<p>\s<\/p>/g,"&nbsp;\n\n").replace(new RegExp(`\\s*</(${blockTags})>\\s*`,"g"),"</$1>\n").replace(new RegExp(`\\s*<((?:${blockTags})(?: [^>]*)?)>`,"g"),"\n<$1>").replace(/(<p [^>]+>.*?)<\/p>/g,"$1</p#>").replace(/<div( [^>]*)?>\s*<p>/gi,"<div$1>\n\n").replace(/\s*<p>/gi,"").replace(/\s*<\/p>\s*/gi,"\n\n").replace(/\n[\s\u00a0]+\n/g,"\n\n").replace(/(\s*)<br ?\/?>\s*/gi,(m,prefix)=>{return prefix&&prefix.includes("\n")?"\n\n":"\n"}).replace(/\s*<div/g,"\n<div").replace(/<\/div>\s*/g,"</div>\n").replace(/\s*\[caption([^\[]+)\[\/caption\]\s*/gi,"\n\n[caption$1[/caption]\n\n").replace(/caption\]\n\n+\[caption/g,"caption]\n\n[caption").replace(new RegExp(`\\s*<((?:${preTags})(?: [^>]*)?)\\s*>`,"g"),"\n<$1>").replace(new RegExp(`\\s*</(${preTags})>\\s*`,"g"),"</$1>\n").replace(/<((li|dt|dd)[^>]*)>/g," \t<$1>");if(text.includes('<option')){text=text.replace(/\s*<option/g,"\n<option").replace(/\s*<\/select>/g,"\n</select>")}
if(text.includes('<hr')){text=text.replace(/\s*<hr( [^>]*)?>\s*/g,"\n\n<hr$1>\n\n")}
if(text.includes('<object')){text=text.replace(/<object[\s\S]+?<\/object>/g,(match)=>{return match.replace(/[\r\n]+/g,"")})}
text=text.replace(/<\/p#>/g,"</p>\n").replace(/\s*(<p [^>]+>[\s\S]*?<\/p>)/g,"\n$1").replace(/^\s+/,"").replace(/[\s\u00a0]+$/,"");if(restoreLineBreak){text=text.replace(/<wp-line-break>/g,"\n")}
if(restoreTagBr){text=text.replace(/<wp-temp-br([^>]*)>/g,"<br$1>")}
if(restoreCode.length){text=text.replace(/<wp-preserve>/g,()=>restoreCode.shift())}
return text}
$usof.field.editor={init:function(){const self=this;self.$container=$('.usof-editor',self.$row);$('script.usof-editor-template',self.$container).remove();self.origEditorId=self.$input.data('editor-id')||'usof_editor';self.origEditorOpts=_window.tinyMCEPreInit.mceInit[self.origEditorId]||{};self.editorSettings={};var $settings=$('.usof-editor-settings',self.$row);if($settings.attr('onclick')){self.editorSettings=$settings[0].onclick()||{}}
$settings.remove();self.editorId=self.origEditorId+$ush.uniqid();self.$input.attr('id',self.editorId);self._events={textareaOnChange:self.textareaOnChange.bind(self),tinymceOnChange:self.tinymceOnChange.bind(self),};self.$container.on((self.isLiveBuilder()?'input':'change'),'textarea',self._events.textareaOnChange);self.createEditor()},createEditor:function(){const self=this;if(!_window.wp||!_window.wp.editor){return}
const isLiveBuilder=self.isLiveBuilder();const theEditorSettings={quicktags:!0,tinymce:self.editorSettings.tinymce||{},mediaButtons:!$ush.isUndefined(self.editorSettings.media_buttons)?self.editorSettings.media_buttons:!0};const qtSettings={id:self.editorId,buttons:"strong,em,link,block,del,ins,img,ul,ol,li,code,more,close",};const usedFields=['content_css','toolbar1','toolbar2','toolbar3','toolbar4','theme','skin','language','formats','relative_urls','remove_script_host','convert_urls','browser_spellcheck','fix_list_elements','entities','entity_encoding','keep_styles','resize','menubar','branding','preview_styles','end_container_on_empty_block','wpeditimage_html5_captions','wp_lang_attr','wp_keep_scroll_position','wp_shortcut_labels','plugins','wpautop','indent','tabfocus_elements','textcolor_map','textcolor_rows','external_plugins',].concat(Object.keys(self.editorSettings.tinymce));_window.tinymce.on('AddEditor',(e)=>{const theEditor=e.editor;const tinymceOnChange='NodeChange '+(isLiveBuilder?'input':'change');if(theEditor.id!==self.editorId){return}
theEditor.off(tinymceOnChange).on(tinymceOnChange,self._events.tinymceOnChange);if(isLiveBuilder){theEditor.on('keydown',self.trigger.bind(self,'tinyMCE.Keydown')).on('BeforeAddUndo',(e)=>{return!$ush.isUndefined(e.origEvent)})}},!0);usedFields.forEach((key)=>{if(!$ush.isUndefined(self.origEditorOpts[key])){theEditorSettings.tinymce[key]=self.origEditorOpts[key]}});theEditorSettings.tinymce.setup=()=>{};self.destroyEditor();_window.wp.editor.initialize(self.editorId,theEditorSettings);_window.quicktags(qtSettings);self.switchEditor('tinymce')},destroyEditor:function(){const self=this;$ush.toArray(_window.tinymce.editors).map((tinymceInstance)=>{if($ush.toString(tinymceInstance.id).indexOf(self.origEditorId)===0){tinymceInstance.destroy()}})},switchEditor:function(mode){const self=this;if($ush.toString(mode).toLowerCase()==='tinymce'){mode='tmce'}else{mode='html'}
$(`#${self.editorId}-${mode}`,self.$container).trigger('click')},textareaOnChange:function(e){this.trigger('change',e.currentTarget.value)},tinymceOnChange:function(e){const self=this;var tinymceContent=_window.tinymce.get(self.editorId).getContent();const currentValue=self.getValue();if(self.origEditorOpts.wpautop){tinymceContent=us_wpnop(tinymceContent)}
if(currentValue.trim()===tinymceContent){return}
self.$input.val(tinymceContent);self.trigger('change',tinymceContent)},getHtmlContent:function(){return us_wpautop(this.getValue())},setValue:function(value,quiet){const self=this;self.$input.val(value);if(_window.tinyMCE&&self.editorId){const mceContent=_window.tinyMCE.get(self.editorId);const mceSetContent=()=>{if(self.origEditorOpts.wpautop){value=us_wpautop(value)}
mceContent.setContent(value)};if(mceContent.initialized){mceSetContent()}else{mceContent.on('init',mceSetContent)}}
...
$usof.instance.setValues(values);this.valuesChanged=values;$usof.instance.saveChanges()}
...
const PREVIEW_TYPE={DYNAMIC_VALUE:'dynamic_value',IMAGE:'image',TEXT:'text',};$usof.field.upload={init:function(options){const self=this;self.parentInit(options);self.$container=$('.usof-upload',self.$row);self.$preview=$('.usof-upload-preview',self.$row);self._dynamicLabels={};self.hasDynamicValues=$('.usof-popup',self.$row).length;self.i18n={};self.isMultiple=self.$container.hasClass('is_multiple');self.placeholder=$('input[name="placeholder"]',self.$row).val();self.popup={};self._previewType=self.$container.data('preview-type');self.$container.removeAttr('data-preview-type');var $i18n=$('.usof-upload-i18n',self.$row);if($i18n.length>0){self.i18n=$i18n[0].onclick()||{};$i18n.remove()}
...
$previewFile.remove()}},selectDynamicValue:function(e){const self=this;e.preventDefault();self.setPreviewType(PREVIEW_TYPE.DYNAMIC_VALUE);self.setValue($ush.toString(e.target.dataset.dynamicValue));if(self.popupId){$usof.hidePopup($ush.toString(self.popupId))}},renderPreview:function(text,value,removeButton,index){const self=this;text=String(text);if(!text){return}
var newPreview='';const previewType=self.getPreviewType();const isPlaceholder=(text===':placeholder');if(previewType===PREVIEW_TYPE.DYNAMIC_VALUE){newPreview='<span>'+$ush.toString(self._dynamicLabels[value])+'</span>'}else if(previewType===PREVIEW_TYPE.IMAGE){if(!isPlaceholder){newPreview='<img src="'+text+'" alt="">'}}else{if(isPlaceholder){text='...'}
...
newPreview=`
			...
			`;$('[data-value="'+value+'"]',self.$preview).remove();var previews=$('> *',self.$preview).toArray();previews.splice($ush.parseInt(index),0,newPreview);self.$preview.append(previews);self.$preview.toggleClass('hidden',!previews.length)},setValue:function(value,quiet){const self=this;value=$ush.toString(value);self.parentSetValue(value,quiet);if(!quiet){self.trigger('change',[value])}
...
return}
if(value.indexOf('{{')>-1){self.setPreviewType(PREVIEW_TYPE.DYNAMIC_VALUE)}
...
$row.removeAttr('onclick');if($row.hasClass('fullwidth-direction-field')){$row.find('.usof-form-row-title').append('<i class="fas fa-unlink"></i>')}else{$row.append('<i class="fas fa-unlink"></i>')}
...
self.$errorMessage.toggleClass('hidden',isValid)},switchRelatedFields:function(e){let self=this,$button=$(e.currentTarget),isDisabled=$button.hasClass('fa-unlink'),mainField=$(e.delegateTarget).data('usofField'),mainValue=mainField.getValue();$button.toggleClass('fa-unlink',!isDisabled).toggleClass('fa-link',isDisabled);mainField.$$relatedFields.map((relatedField)=>{if(mainField.$row.hasClass('fullwidth-direction-field')){if(relatedField!==mainField){if(isDisabled){relatedField.$text[0].setAttribute('disabled','disabled')}else{relatedField.$text[0].removeAttribute('disabled')}}}
if(isDisabled){if(relatedField!==mainField){relatedField.setValue(mainValue)}
...
if(!self.isWPBakeryParamValue()){$('> div',self.$container).removeClass('active')}},forceWPBValue:function(){var self=this;if(self.hasStringValue){self.setValue(self.getValue())}}}}(jQuery);!function($,undefined){var _window=window,_document=document;if(_window.$usof===undefined){return}
...
return ''},applyStyles:function(){const self=this;const classRandomPart=$ush.uniqid();var className=`.usof-btn_${classRandomPart}`,style={default:'',hover:'',};self.$button.usMod('usof-btn',classRandomPart);self.$button.usMod('hov',self.groupParams.getValue('hover'));self.$button.usMod('hovText',self.groupParams.getValue('hover_text_animation'));var buttonFont=self.groupParams.getValue('font'),typographyOptions=$usof.getData('typographyOptions')||{},fontFamily;if($.inArray(buttonFont,Object.keys(typographyOptions))!==-1){fontFamily=(typographyOptions[buttonFont]||{})['font-family']||((typographyOptions[buttonFont]||{}).default||{})['font-family']||''}else{fontFamily=buttonFont}
if(fontFamily!=='none'&&fontFamily!==''&&fontFamily!=='null'){style.default+='font-family: '+fontFamily+'!important;'}
if(self.groupParams.getValue('text_style').includes('italic')){style.default+='font-style: italic !important;'}else{style.default+='font-style: normal !important;'}
if(self.groupParams.getValue('text_style').includes('uppercase')){style.default+='text-transform: uppercase !important;'}else{style.default+='text-transform: none !important;'}
style.default+='font-size: min('+self.groupParams.getValue('font_size')+', 50px) !important;';style.default+='line-height:'+self.groupParams.getValue('line_height')+' !important;';style.default+='font-weight:'+self.groupParams.getValue('font_weight')+' !important;';style.default+='padding:'+self.groupParams.getValue('height')+' '+self.groupParams.getValue('width')+' !important;';style.default+='border-radius:'+self.groupParams.getValue('border_radius')+' !important;';style.default+='letter-spacing:'+self.groupParams.getValue('letter_spacing')+' !important;';style.default+='transition-timing-function:'+self.groupParams.getValue('transition_timing_function')+' !important;';var btnLineHeight=self.groupParams.getValue('line_height');if(btnLineHeight.includes('px')){btnLineHeight+='em'}
style.default+='--btn-height: calc('+btnLineHeight+' + 2 * '+self.groupParams.getValue('height')+');';style.default+='--btn-transition-duration:'+self.groupParams.getValue('transition_duration')+';';var colorBg=self._getColorValue('color_bg'),colorBorder=self._getColorValue('color_border'),colorBgHover=self._getColorValue('color_bg_hover'),colorBorderHover=self._getColorValue('color_border_hover'),color;if(colorBg==''){colorBg='transparent'}
...
if(self.groupParams.getValue('hover')=='circle'){style.default+='background: transparent !important;'}else if(self.groupParams.getValue('hover')=='scaleDown'){style.default+='background:'+colorBgHover+' !important;'}else{style.default+='background:'+colorBg+' !important;'}
if(colorBorder.includes('gradient')){style.default+='border-image:'+colorBorder+' 1;';style.default+='border-color: transparent;'}else{style.default+='border-color:'+colorBorder+';';style.default+='border-image: none;'}
...
if(self.groupParams.getValue('hover')=='fade'&&!colorBg.includes('gradient')&&(colorBgHover=='transparent'||colorBgHover=='rgba(0,0,0,0)')){style.hover+='background:'+colorBgHover+' !important;'}
if(colorBorderHover.includes('gradient')){style.hover+='border-image:'+colorBorderHover+' 1;';style.hover+='border-color: transparent;'}else{style.hover+='border-color:'+colorBorderHover+';';style.hover+='border-image: none;'}
var colorHover;if(self._getColorValue('color_text_hover').includes('gradient')){colorHover=usofColorAPI.gradientParse(self._getColorValue('color_text_hover')).firstValue}else{colorHover=self._getColorValue('color_text_hover')}
style.hover+='color:'+colorHover+' !important;';var compiledStyle=className+'{%s}'.replace('%s',style.default);if(self._getColorValue('color_shadow_hover')!=''){style.hover+='box-shadow:'+self.groupParams.getValue('shadow_hover_offset_h')+' '+self.groupParams.getValue('shadow_hover_offset_v')+' '+self.groupParams.getValue('shadow_hover_blur')+' '+self.groupParams.getValue('shadow_hover_spread')+' '+self._getColorValue('color_shadow_hover')+' ';if($.inArray('1',self.groupParams.getValue('shadow_hover_inset'))!==-1){style.hover+='inset'}
style.hover+='!important;'}
compiledStyle+=className+':before {border-width:'+self.groupParams.getValue('border_width')+' !important;}';compiledStyle+=className+':hover{%s}'.replace('%s',style.hover);if(self.groupParams.getValue('hover')=='circle'){compiledStyle+=className+'::after {background:'+colorBg+'!important;}';compiledStyle+=className+':hover::after {background:'+colorBgHover+'!important;}'}else if(self.groupParams.getValue('hover')=='scaleDown'){compiledStyle+=className+'::after {background:'+colorBg+'!important;}'}else{compiledStyle+=className+'::after {background:'+colorBgHover+'!important;}'}
